...
print('Total Pay: $','%.2f' % Pay, sep='')  # '%.2f' forces 2 decimal places;
                                            # sep'' removes the space between strings
...
# Need to import the Regular Expression module
import re

sampstr = ['X-S: CMU 2.3','X-D: Inn','X-CON: 0.87','Y-Ty: pl']

# returns 1st 3 strings as they start with X (^X), match any character (.), any
# number of times (*) until it hits a colon (:)
for item in sampstr:
    x = re.findall('^X.*:',item)
    print(x)
    
# returns a list of items that match the range (0-9) and have 1 or more times (+)
w = 'My 2 favorite numbers are 88 and 95'
x = re.findall('[0-9]+',w)
print(x)
# result is [2, 88, 95] - Note these are strings

xnongreedy = re.findall('[0-9]+?',w)
print(xnongreedy)
# result is [2, 8, 8, 9, 5]

y = re.findall('[a-z]+',w)
print(y)
# result is ['y', 'favorite', 'numbers', 'are', 'and']

z = re.findall('[A-z]+',w)
print(z)
# result is ['My', 'favorite', 'numbers', 'are', 'and']

# searching for characters before and after the @ to extract an e-mail address
w = 'From steven.smith@cbchs.org Sat Jun 02 09:34:16 2017'
x = re.findall('\S+@\S+',w)
print(x)
#result is a list of a single item ['steven.smith@cbchs.org']

# Parenthesis are not part of the match - but they tell where to start and stop
# In this case it starts at the 'From ' but is not included in the extract
w = 'To bart@bartcross.com From steven.smith@cbchs.org Sat Jun 02 09:34:16 2017'
x = re.findall('From (\S+@\S+)',w)
print(x)
# result is ['steven.smith@cbchs.org']
...
import os
os.chdir(r'c:\Users\public\Desktop')  # Changes working directory. Use 'r' in the string 
									  # so the backslashes are treated literally
ie = ('Internet Explorer.lnk')
os.remove(ess)						# Deletes a file
...
print('Name:', tree.find('name').text)          # Returns 'Chuck'
print('Attr:', tree.find('email').get('hide'))  # Returns 'yes'
